...
int main()
{
    ...
    int pd=arr[1]-arr[0];
    int current=2;   //current here because 0 th and 1st is already in pd
    int ans=2;    // initial  current lengh is only the answer till now,, isliye we stored 2 already 
    int j=2; // we will start iterating from 2 as we have already covered till j=2
    while(j<n){
        if(pd==arr[j]-arr[j-1]){
            current++;
        }
